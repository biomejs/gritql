...
- `level`: (Optional, one of `none`, `info`, `warn`, `error`) The enforcement level. Defaults to `info`.
- `tags`: (Optional, `string[]`) A list of tags for filtering patterns.
