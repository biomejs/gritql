...
| `call_expression()` | **[AST nodes](/language/patterns#syntax-tree-nodes)** can also be used as patterns. |
| `call_expression(callee=$callee)` | AST nodes can specify **fields**, where each field value is bound to a pattern or metavariable. |
| `` `console.log($message)` => `console.warn($message)` `` | A **[rewrite](/language/patterns#rewrite-operator)** is a pattern followed by a `=>` followed by a pattern. Rewrites are usable as patterns. |
| `$x => .` | The `.` is an **empty/null pattern**, it replaces the matched node with nothing. |
...
| `` after `console.warn($_)` `` | **`after` clauses** search for a pattern that occurs after another pattern. |
...
| `` `console.log($message)` where { $new = "Hello", $new += "world" } => $new `` | Strings and lists can be appended to using `+=`. |
...
| `` `console.log` limit 10 `` | The [**`limit` clause**](/language/modifiers#limit-clause) can be used to limit the number of files returned by a query. |
...
| `` function custom_example($name) { return `name: $name` } `` | **[Custom functions](/language/functions)** can be used to define reusable right-side values. |
